...
def create_update_script(current_exe: Path, downloaded_exe: Path) -> Path:
    ...
    side_by_side_exe = current_exe.parent / downloaded_exe.name
    lines = [
        "@echo off",
        "setlocal",
        f'set "TARGET={current_exe}"',
        f'set "SOURCE={downloaded_exe}"',
        f'set "SIDE_BY_SIDE={side_by_side_exe}"',
        f'set "AVAILABLE={downloaded_exe}"',
        f'set "PID={os.getpid()}"',
        f'set "LOG={log_path}"',
        'echo [%DATE% %TIME%] Starting XML Editor Desktop update > "%LOG%"',
        'echo Target: "%TARGET%" >> "%LOG%"',
        'echo Source: "%SOURCE%" >> "%LOG%"',
        'echo Side-by-side fallback: "%SIDE_BY_SIDE%" >> "%LOG%"',
        ...
        "  goto end",
        ")",
        'echo Saving update beside the current executable. >> "%LOG%"',
        'copy /Y "%SOURCE%" "%SIDE_BY_SIDE%" >> "%LOG%" 2>&1',
        "if errorlevel 1 (",
        '  echo Side-by-side save failed; keeping temp download. >> "%LOG%"',
        ") else (",
        '  set "AVAILABLE=%SIDE_BY_SIDE%"',
        ")",
        'copy /Y "%SOURCE%" "%TARGET%" >> "%LOG%" 2>&1',
        "if errorlevel 1 (",
        '  echo Replacement failed with errorlevel %ERRORLEVEL%. >> "%LOG%"',
        '  echo Update remains available at "%AVAILABLE%". >> "%LOG%"',
        '  start "" explorer.exe /select,"%AVAILABLE%"',
        "  goto end",
        ")",
        'del "%SOURCE%" >> "%LOG%" 2>&1',
        'if /I not "%AVAILABLE%"=="%SOURCE%" if /I not "%AVAILABLE%"=="%TARGET%" del "%AVAILABLE%" >> "%LOG%" 2>&1',
